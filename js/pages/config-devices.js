...
async function determineActiveInstitution() {
  if (isSuperAdmin()) {
    if (!deviceManagementFilter) {
      ...
      deviceManagementFilter = activeGlobal || {
        id: "all",
    ...
    if (!getActiveInstitution() && userInsts.length === 1) {
      const inst = hierarchyCache.instituicoes?.find(
        (i) => i.id === userInsts[0],
      );
  ...
  try {
    ...
    if (isSuperAdmin()) {
      if (deviceManagementFilter?.id && deviceManagementFilter.id !== "all") {
        q = query(
          devicesRef,
          where("instituicaoID", "==", deviceManagementFilter.id),
        );
      } else {
        q = query(devicesRef);
      }
...
function renderMainView() {
  const currentLabel =
    deviceManagementFilter?.id === "all"
      ? "Todas as Instituições"
      : deviceManagementFilter?.nome || "Selecione";

  const filterHTML =
    isSuperAdmin() && hierarchyCache.instituicoes?.length > 0
...
function renderDevicesGrid(devices) {
  if (devices.length === 0)
    return `<div class="empty-state"><i class="fas fa-microchip"></i><p>Nenhum dispositivo.</p></div>`;
      ...
      ${devices
        .map((device) => {
          ...
          const unit = hierarchyCache.unidades?.find(
            (u) => u.id === device.unidadeID,
          );
          const unitName = unit
            ? unit.nome
            : device.nomeUnidade || "Unidade N/A";

          const sector = hierarchyCache.setores?.find(
            (s) => s.id === device.setorID,
          );
          const sectorName = sector
            ? sector.nome
            : device.nomeSetor || "Setor N/A";
          ...
          const cAlarmeSonda =
            device.alarmeSondaAtivo !== false ? "#ef4444" : "#cbd5e1";
          const cAlarmeAmb =
            device.alarmeTempAmbienteAtivo !== false ? "#f59e0b" : "#cbd5e1";
          const cAlarmeUmid =
            device.alarmeUmidadeAtivo !== false ? "#3b82f6" : "#cbd5e1";

          const minSonda = device.alarmeMin?.sonda ?? "-";
          const maxSonda = device.alarmeMax?.sonda ?? "-";
          const minTempAmbiente = device.alarmeMin?.temperaturaAmbiente ?? "-";
          const maxTempAmbiente = device.alarmeMax?.temperaturaAmbiente ?? "-";
          const minUmidade = device.alarmeMin?.umidade ?? "-";
          const maxUmidade = device.alarmeMax?.umidade ?? "-";
          ...
          <div class="device-card ${statusClass}">
              ...
              <div class="device-body">
                  ...
                  <div class="data-row">
                      <small>Limites T.Sonda</small>
                      <strong>${minSonda}°C <i class="fas fa-arrows-alt-h" style="font-size:0.7em; color:#ccc;"></i> ${maxSonda}°C</strong>
                  </div>
                  <div class="data-row">
                      <small>Limites T. Ambiente</small>
                      <strong>${minTempAmbiente}°C <i class="fas fa-arrows-alt-h" style="font-size:0.7em; color:#ccc;"></i> ${maxTempAmbiente}°C</strong>
                  </div>
                  <div class="data-row">
...
async function openDeviceModal(deviceId) {
  ...
  const closeModal = () => {
    if (document.body.contains(modalOverlay)) modalOverlay.remove();
  };
  modalOverlay
...
function setupViewEvents() {
  ...
  if (instFilter) {
    instFilter.addEventListener("change", async (e) => {
      ...
      deviceManagementFilter =
        val === "all"
          ? { id: "all" }
          : hierarchyCache.instituicoes.find((x) => x.id === val);
      await loadDevices();
    });
  }
  ...
  // 3. Edição
  document
    .getElementById("devices-list-wrapper")
