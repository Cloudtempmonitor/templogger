...
export class DeviceChartManager {
  ...
  _processData(readings) {
    ...
    dataToProcess.forEach((reading) => {
      ...
      // FILTRO DE ERRO DE SONDA (-127)
      const tempSonda = reading.temperatura;
      const tempAmb = reading.temperaturaAmbiente;
      const umid = reading.umidade;

      // Aplica a lógica: Se for -127 ou nulo, vira null. Senão, valor.
      temperatures.push(tempSonda === -127 ? null : (tempSonda ?? null));
      ambientTemps.push(tempAmb === -127 ? null : (tempAmb ?? null));
      humidities.push(umid === -127 ? null : (umid ?? null));
